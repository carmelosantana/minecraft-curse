...
# The Curse Plugin Server Manager
...
SERVER_DIR="server"
PLUGIN_NAME="curse"
PLUGIN_JAR="target/${PLUGIN_NAME}-0.1.3.jar"
...
online-mode=false
white-list=false
motd=The Curse Plugin Test Server
view-distance=10
...
EOF
    ...
    print_status "Note: Use 'make give-bad-omen' for testing commands"
}
...
show_status() {
    ...
    # Check plugin status
    if [[ -f "$SERVER_DIR/plugins/${PLUGIN_NAME}-0.1.3.jar" ]]; then
...
case "${1:-help}" in
    ...
    players)
        ...
        ;;
    help|*)
        echo "The Curse Plugin Server Manager"
        echo ""
