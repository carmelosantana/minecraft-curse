...
public class LeaderboardManager {
    ...
    /**
     * Update player statistics for any curse activity (including new mechanics)
     */
    public void updatePlayerStats(UUID playerId, CurseActivity activity) {
        Player player = plugin.getServer().getPlayer(playerId);
        if (player == null) return;

        // Update player's persistent data
        updatePlayerPersistentData(player, activity);

        // Update server-wide leaderboard
        updateServerLeaderboard(player, activity);
    }
    ...
    /**
     * Update player persistent data for any curse activity
     */
    private void updatePlayerPersistentData(Player player, CurseActivity activity) {
        var pdc = player.getPersistentDataContainer();

        // Total completions
        int completions = pdc.getOrDefault(totalCompletionsKey, PersistentDataType.INTEGER, 0);
        if (activity.isSuccessful()) { // Count successful activities as completions
            completions++;
            pdc.set(totalCompletionsKey, PersistentDataType.INTEGER, completions);
        }
        ...
        // Total kills
        int totalKills = pdc.getOrDefault(totalKillsKey, PersistentDataType.INTEGER, 0);
        totalKills += activity.getTotalKills();
        pdc.set(totalKillsKey, PersistentDataType.INTEGER, totalKills);

        // Best time (for successful activities)
        if (activity.isSuccessful()) {
            long currentTime = System.currentTimeMillis() - activity.getStartTime();
            long bestTime = pdc.getOrDefault(bestTimeKey, PersistentDataType.LONG, Long.MAX_VALUE);
            if (currentTime < bestTime) {
                pdc.set(bestTimeKey, PersistentDataType.LONG, currentTime);
            }
        ...
        // Highest round reached
        int highestRound = pdc.getOrDefault(highestRoundKey, PersistentDataType.INTEGER, 0);
        if (activity.getCurrentRound() > highestRound) {
            pdc.set(highestRoundKey, PersistentDataType.INTEGER, activity.getCurrentRound());
        }
    }

    /**
     * Update server leaderboard for any curse activity
     */
    private void updateServerLeaderboard(Player player, CurseActivity activity) {
        String playerName = player.getName();
        String path = "players." + playerName;
        ...
        globalKills += activity.getTotalKills();
        ...
        playerKills += activity.getTotalKills();
        ...
        int highestRound = leaderboardConfig.getInt(path + ".highestRound", 0);
        if (activity.getCurrentRound() > highestRound) {
            leaderboardConfig.set(path + ".highestRound", activity.getCurrentRound());
        }
